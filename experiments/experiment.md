...
- 서브트리 캐싱
  ...
  - 각 테넌트는 캐싱된 동일한 서브트리를 사용하여 불필요한 시스템 오버해드를 방지합니다.
...
각 네임스페이스별로 TopK를 1, 3, 5로 변화를 주며 질문하였고, 위 과정을 3회 반복하여 결과의 신뢰성을 확보하였습니다.
...
- Context Recall
  ...
  - TopK 나 Chunk Size 변화에 따라 검색 범위가 달라질때 모범 담안에 포함된 개념이 검색 결과에 얼마나 포함되어 있는지를 평가합니다.
- Context Precision
...
TopK 가 증가함에 따라 검색 단게에서 더 많은 문서 청크가 포함되므로 TopK 증가에 따라  Context Recall이 증가할 것으로 예상됩니다.
...
Chunk Size가 큰 경우 Overlap 증가와 관계 없이 이미 하나의 문서 청크 내부에 정답과 관련된 충분한 정보가 들어있기 때문에 유의미한 Conext Recall 증가는 없을 것으로 예상됩니다.
...
TopK 실험 결과, TopK가 1인 경우 Answer Relevancy(0.61), Context Precision(0.66), Context Recall(0.71) 모두 가장 낮은 값을 보였습니다.
...
반면 topK가 크면 검색 결과에 질문과 관련 없는 노이즈가 포함되어 응답 품질이 저하되는 현상이 관찰됩니다.
본 실험 환경에서는 topK가 3인 경우에서 충분한 청크 개수와 노이즈 사이에 균형점을 이루는 최적의 설정으로 평가됩니다.


### Chunk Size

Chunk Size 실험 결과, Chunk Size가 256인 경우 Context Precision(0.49), Context Recall(0.66), Answer Relevancy(0.63) 모두 가장 낮은 값을 보였습니다.
작은 청크 크기로 인해 각 청크가 모범 답변에 포함된 충분한 정보를 포함하지 못한 것으로 관찰됩니다.
또한 작은 청크 크기로 인해 질문에 대한 답변을 생성할 충분한 정보를 포함하지 못해 답변 품질이 저하되었다고 해석할 수 있습니다.

Chunk Size를 512로 설정한 경우 Context Precision(0.82), Context Recall(0.87), Answer Relevancy(0.69) 모두 급격하게 증가하였습니다.
청크가 적절한 크기로 설정되어 청크 내부에 노이즈가 적으면서도 모범 답변에 포함된 충분한 정보를 포함한 것이 관찰됩니다.
결과적으로 생성 단계에서 질문에 대한 답변을 생성할 충분한 정보가 제공되면서 높은 답변 품질(Answer Relevancy)을 보였다고 해석할 수 있습니다.
...
따라서 RAGAS는 유용한 청크로 판단했지만, 실제로는 큰 청크 내부에 질문과 관련 없는 노이즈 정보들이 많이 포함되어 있던 것이 관찰됩니다.
이러한 노이즈 정보로 인해 답변 품질(Answer Relevancy)이 저하되는 결과를 보였다고 해석됩니다.
...
본 실험 환경에서는 Chunk Size가 512인 경우가 충분한 정보량과 노이즈 사이에 균형점을 이루는 최적의 설정으로 평가됩니다.

### Overlap

Overlap 실험 결과, Context Recall(0.87)은 Overlap 값과 관계없이 일관된 수치를 보였습니다.
그러므로 Overlap의 변화가 모범 답변에 포함된 핵심 개념 검색에는 큰 영향을 미치지지 않는다고 해석할 수 있습니다.

Overlap이 없는 경우 Context Precision(0.76)과 Answer Relevancy(0.64)가 상대적으로 낮은 값을 보였습니다.
Overlap이 없는 경우 청크 경계에서 문맥이 단절되어, 검색된 청크가 답변에 필요한 정보를 포함하지 못하는 것을 관찰하였습니다.
...
이러한 노이즈 청크들은 질문과 관련된 핵심 키워드를 포함하고 있는 경우가 많아 Retriver 검색 결과 상위에 포함되지만, 답변 생성에 필요하지 않은 핵심 정보들은 포함하지 않는 경우가 많습니다.
그 결과 노이즈 청크들이 상위 검색 결과를 독점하게 되어, 다양한 정보를 가진 청크를 검색하지 못하게 되어 Context Precision과 답변 품질(Answer Relevancy) 이 저하된 것으로 해석할 수 있습니다.

종합하자면, Overlap이 없으면 청크 경계에서 문맥이 단절되어 검색 품질이 저하되고, 과도한 Overlap은 노이즈 청크의 증가로 오히려 성능이 감소합니다. 본 실험에서는 15%의 Overlap이 문맥 연속성 확보와 노이즈 최소화 사이의 최적 균형점으로 평가됩니다.
...
본 실험에서는 환각 현상 문제가 RAG 성능의 병목이 아니라는 것을 확인하였습니다.
